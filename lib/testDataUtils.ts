...
// Helper function for array-based generation (large datasets)
function generateLargeDataset(size1: number, size2: number, commonCount: number) {
...
// Helper function for set-based generation (smaller datasets)
function generateSmallDataset(size1: number, size2: number, commonCount: number) {
  ...
  // Convert sets to arrays and shuffle
...
export const generateTestData = (size1: number, size2: number, intersectionPercent: number) => {
  ...
  const SET_SIZE_LIMIT = 5000000; // Conservative limit for Set size (5M)
  ...
  // Calculate number of common emails
  ...
  // Choose generation strategy based on dataset size
  const useArrayGeneration = size1 > SET_SIZE_LIMIT || size2 > SET_SIZE_LIMIT;
  
  return useArrayGeneration 
    ? generateLargeDataset(size1, size2, commonCount)
    : generateSmallDataset(size1, size2, commonCount);
};
